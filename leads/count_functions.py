...
def Total_Routes_Count(a,lead_status):
    ...
    """
    if(a[0]=='Other'):
        count = Leads.objects.filter(LocationFromOther = a[2],LocationTo = a[1],Lead_Status=lead_status).all().count()
    else:
        count = Leads.objects.filter(LocationFrom = a[0],LocationTo = a[1],Lead_Status=lead_status).all().count()
    return count